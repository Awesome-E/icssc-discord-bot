...
pub(crate) fn generate_add_calendar_link(ixn: &CommandInteraction, calendar_id: String) -> Result<String, BotError> {
    let root_url = std::env::var("RAILWAY_PUBLIC_DOMAIN").context("Missing Domain")?;
    
    let jwt = AddCalendarInteractionTrigger {
        guild_id: ixn.guild_id.map(|v| v.get()).context("Generate oauth link => guild id")?,
        interaction_token: ixn.token.clone(),
        calendar_id
    };
    
    let encoding_key = jsonwebtoken::EncodingKey::from_secret(
        std::env::var_os("JWT_SECRET").context("Missing JWT_SECRET")?.as_encoded_bytes()
    );
